...
public abstract class CmdFactionsPowerBoostAbstract extends FactionsCommand {
	...
	private boolean trySet(FactionsParticipator factionsParticipator, Double powerBoost) throws MassiveException {
		...
		if (!Perm.POWERBOOST_SET.has(sender, true)) {
	...
	private void informPowerBoost(FactionsParticipator factionsParticipator, Double powerBoost, boolean updated) {
		...
		powerBoost = Double
				.valueOf(powerBoost == null ? factionsParticipator.getPowerBoost() : powerBoost.doubleValue());
		String powerDescription = Txt
				.parse(Double.compare(powerBoost.doubleValue(), 0.0D) >= 0 ? "<g>bonus" : "<b>penalty");
		String when = updated ? "now " : "";
		String verb = factionsParticipator.equals(msender) ? "have" : "has";

		String messagePlayer = Txt.parse("<i>%s<i> %s%s a power %s<i> of <h>%.2f<i> to min and max power levels.",
				new Object[] { participatorDescribe, when, verb, powerDescription, powerBoost });
		String messageLog = Txt.parse("%s %s set the power %s<i> for %s<i> to <h>%.2f<i>.",
				new Object[] { msender.getName(), verb, powerDescription, factionsParticipator.getName(), powerBoost });
 ...
 * CmdFactionsPowerBoostAbstract.class Java compiler version: 8 (52.0) JD-Core
 * Version: 0.7.1
 */