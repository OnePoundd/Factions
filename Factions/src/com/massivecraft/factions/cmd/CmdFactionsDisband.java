...
public class CmdFactionsDisband extends FactionsCommand {
	...
	public void perform() throws MassiveException {
		...
		for (MPlayer mplayer : faction.getMPlayers()) {
			EventFactionsMembershipChange membershipChangeEvent = new EventFactionsMembershipChange(sender, mplayer,
					FactionColl.get().getNone(), EventFactionsMembershipChange.MembershipChangeReason.DISBAND);
		...
		if (MConf.get().logFactionDisband) {
			Factions.get()
					.log(new Object[] { Txt.parse("<i>The faction <h>%s <i>(<h>%s<i>) was disbanded by <h>%s<i>.",
							new Object[] { faction.getName(), faction.getId(),
									msender.getDisplayName(IdUtil.getConsole()) }) });
		}
		faction.detach();
	}
}
 ...
 * CmdFactionsDisband.class Java compiler version: 8 (52.0) JD-Core Version:
 * 0.7.1
 */