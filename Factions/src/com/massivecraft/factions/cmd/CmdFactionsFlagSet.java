...
public class CmdFactionsFlagSet extends FactionsCommand {
	...
	public void perform() throws MassiveException {
		...
		if (faction.getFlag(flag) == value) {
			msg("%s <i>already has %s <i>set to %s<i>.",
					new Object[] { faction.describeTo(msender),
							flag.getStateDesc(value, false, true, true, false, true),
							flag.getStateDesc(value, true, true, false, false, false) });
			return;
		}
		...
		if (msender.getFaction() != faction) {
			msg("<h>%s <i>set a flag for <h>%s<i>.",
					new Object[] { msender.describeTo(msender, true), faction.describeTo(msender, true) });
			message(stateInfo);
		}
		faction.msg("<h>%s <i>set a flag for <h>%s<i>.",
				new Object[] { msender.describeTo(faction, true), faction.describeTo(faction, true) });
 ...
 * CmdFactionsFlagSet.class Java compiler version: 8 (52.0) JD-Core Version:
 * 0.7.1
 */