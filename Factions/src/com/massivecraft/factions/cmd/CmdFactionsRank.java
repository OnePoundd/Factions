...
public class CmdFactionsRank extends FactionsCommand {
	...
	private void ensureAllowed() throws MassiveException {
		...
		if (endFaction.isNone()) {
			throw new MassiveException().addMsg("%s <b>doesn't use ranks sorry :(",
					new Object[] { targetFaction.getName() });
		}
		if (target == msender) {
			throw new MassiveException().addMsg("<b>The target player mustn't be yourself.");
		}
		if (targetFaction != msenderFaction) {
			throw new MassiveException().addMsg("%s <b>is not in the same faction as you.",
					new Object[] { target.describeTo(msender, true) });
		}
		if (factionChange) {
			throw new MassiveException().addMsg("<b>You can't change %s's <b>faction.",
					new Object[] { target.describeTo(msender) });
		}
		if (senderRank.isLessThan(rankReq)) {
			throw new MassiveException().addMsg("<b>You must be <h>%s <b>or higher to change ranks.",
					new Object[] { Txt.getNicedEnum(rankReq).toLowerCase() });
		}
		if (senderRank == targetRank) {
			throw new MassiveException().addMsg("<h>%s <b>can't manage eachother.",
					new Object[] { Txt.getNicedEnum(rankReq) + "s" });
		}
	...
	private void ensureMakesSense() throws MassiveException {
		if (target.getRole() == rank) {
			throw new MassiveException().addMsg("%s <b>is already %s.",
					new Object[] { target.describeTo(msender), rank.getDescPlayerOne() });
		}
	}
	...
	private void showRank() {
		...
		if (targetFaction.isNone()) {
			...
			msg("%s <i>%s %s <h>%s <i>%s %s<i>.",
					new Object[] { targetName, isAre, theAan, rankName, ofIn, factionName });
		}
	}
	...
	private void changeFaction() throws MassiveException {
		...
		EventFactionsMembershipChange membershipChangeEvent = new EventFactionsMembershipChange(sender, msender,
				endFaction, EventFactionsMembershipChange.MembershipChangeReason.RANK);
		...
		for (MPlayer recipient : recipients) {
			recipient.msg("%s <i>was moved from <i>%s to <i>%s<i>.", new Object[] { target.describeTo(recipient),
					targetFaction.describeTo(recipient), endFaction.describeTo(recipient) });
		}
		if (MConf.get().logFactionJoin) {
			Factions.get().log(new Object[] { Txt.parse("%s moved %s from %s to %s.", new Object[] { msender.getName(),
					target.getName(), targetFaction.getName(), endFaction.getName() }) });
		}
		targetFaction = target.getFaction();
		targetRank = target.getRole();
		senderRank = msender.getRole();
	...
	private void changeRankLeader() {
		...
		if (targetFactionCurrentLeader != null) {
			...
			if (targetFactionCurrentLeader != msender) {
				targetFactionCurrentLeader.msg("<i>You have been demoted from the position of faction leader by %s<i>.",
						new Object[] { msender.describeTo(targetFactionCurrentLeader, true) });
			}
		}
		...
		for (MPlayer recipient : MPlayerColl.get().getAllOnline()) {
			...
			recipient.msg("%s<i> gave %s<i> the leadership of %s<i>.",
					new Object[] { changerName, target.describeTo(recipient), targetFaction.describeTo(recipient) });
		}
	}

	private void changeRankOther() throws MassiveException {
		if ((targetRank == Rel.LEADER)
				&& ((!MConf.get().permanentFactionsDisableLeaderPromotion) || (!targetFaction.getFlag("permanent")))
				&& (targetFaction.getMPlayersWhereRole(Rel.LEADER).size() == 1)) {
			targetFaction.promoteNewLeader();
			if (targetFaction.detached()) {
				target.resetFactionData();
				throw new MassiveException().addMsg(
						"<i>The target was a leader and got demoted. The faction disbanded and no rank was set.");
			}
		}
		Set<MPlayer> recipients = new HashSet();
		...
		for (MPlayer recipient : recipients) {
			...
			recipient.msg("%s<i> %s %s from %s to <h>%s <i>in %s<i>.", new Object[] { targetName, wasWere, change,
					oldRankName, rankName, targetFaction.describeTo(msender) });
		}
	}
}
 ...
 * CmdFactionsRank.class Java compiler version: 8 (52.0) JD-Core Version: 0.7.1
 */