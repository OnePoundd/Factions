...
public class CmdFactionsPermSet extends FactionsCommand {
	...
	public void perform() throws MassiveException {
		...
		if (faction.getPermitted(perm).contains(rel) == value.booleanValue()) {
			msg("%s <i>already has %s <i>set to %s <i>for %s<i>.",
					new Object[] { faction.describeTo(msender), perm.getDesc(true, false),
							Txt.parse(value.booleanValue() ? "<g>YES" : "<b>NOO"),
		...
		for (MPlayer recipient : recipients) {
			recipient.msg("<h>%s <i>set a perm for <h>%s<i>.",
					new Object[] { msender.describeTo(recipient, true), faction.describeTo(recipient, true) });
 ...
 * CmdFactionsPermSet.class Java compiler version: 8 (52.0) JD-Core Version:
 * 0.7.1
 */