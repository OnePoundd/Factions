...
public class TypeRank extends TypeEnum<Rel> {
	...
	static {
		...
		Rel[] arrayOfRel;
		int k = (arrayOfRel = Rel.values()).length;
		for (int j = 0; j < k; j++) {
			Rel rel = arrayOfRel[j];
			if (rel.isRank()) {
	...
	public static TypeRank get(Rel rank) {
		return (TypeRank) instances.get(rank);
	}
	...
	public Set<String> getNamesInner(Rel value) {
		...
		if (start != null) {
			if ((value == Rel.LEADER) && (start == Rel.OFFICER)) {
				ret.addAll(NAMES_PROMOTE);
			}
			if ((value == Rel.OFFICER) && (start == Rel.MEMBER)) {
				ret.addAll(NAMES_PROMOTE);
			}
			if ((value == Rel.OFFICER) && (start == Rel.LEADER)) {
				ret.addAll(NAMES_DEMOTE);
			}
			if ((value == Rel.MEMBER) && (start == Rel.RECRUIT)) {
				ret.addAll(NAMES_PROMOTE);
			}
			if ((value == Rel.MEMBER) && (start == Rel.OFFICER)) {
				ret.addAll(NAMES_DEMOTE);
			}
			if ((value == Rel.RECRUIT) && (start == Rel.MEMBER)) {
				ret.addAll(NAMES_DEMOTE);
			}
		}
		return ret;
 ...
 * TypeRank.class Java compiler version: 8 (52.0) JD-Core Version: 0.7.1
 */